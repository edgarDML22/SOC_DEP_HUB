...
class ResultadoController extends Controller
{
    ...
    {
        if (auth()->user()->rol !== 'instrcutor' || auth()->user()->rol !== 'INSTRUCTOR') {
            return response()->json([
    ...
    public function resultadosPendientes()
    {
        if (auth()->user()->rol !== 'subgerente' || auth()->user()->rol !== 'SUBGERENTE') {
            return response()->json([
                'message' => 'No autorizado'
            ], 403);
        }
        $encuentros = EncuentrosTorneo::where(
            'estatus_encuentro',

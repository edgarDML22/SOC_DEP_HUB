...
<template>
  <div class="flex flex-col gap-6 font-sans">
    ...
    <div class="w-full md:max-w-md">
      <input v-model="search" placeholder="Buscar por nombre de menor..."
    ...
    <!-- MENSAJE -->
    <div v-if="mensaje"
      :class="tipoMensaje === 'error' ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'"
      class="p-4 rounded-xl border text-sm font-semibold shadow-sm animate-pulse">
      {{ mensaje }}
    </div>
    ...
    <!-- Empty State -->
    <div v-else-if="miembrosFiltrados.length === 0"
      class="bg-white rounded-2xl md:rounded-3xl p-8 md:p-12 text-center text-surface-600 shadow-sm border border-surface-200 flex flex-col items-center justify-center min-h-[250px]">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-surface-300 mb-4" fill="none" viewBox="0 0 24 24"
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <div v-for="m in miembrosFiltrados" :key="m.id_registro"
        class="bg-white rounded-2xl md:rounded-3xl p-6 shadow-sm border border-surface-200 transition-all hover:shadow-md hover:border-primary-200 flex flex-col h-full group pb-7">

        <!-- Header tarjeta -->
        <div class="flex items-start gap-4 mb-5">
          <div
            class="w-14 h-14 rounded-2xl flex items-center justify-center bg-primary-50 text-primary-700 font-bold text-xl uppercase shrink-0 border border-primary-100 shadow-sm group-hover:scale-105 transition-transform">
            {{ m.menor?.charAt(0) || '?' }}
          </div>
        ...
        <div class="flex-1 flex flex-col gap-2.5 text-sm text-surface-600 font-medium mb-6">
          <div
            class="flex items-center justify-between p-3 rounded-xl bg-surface-50 border border-surface-100 group-hover:border-surface-200 transition-colors">
        ...
        <div v-if="esGerente" class="mt-auto flex gap-3 flex-col sm:flex-row w-full pt-3 border-t border-surface-100">
          <button v-if="m.estatus_visita !== 'ACTIVA'" type="button"

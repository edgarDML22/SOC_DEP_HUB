...
// Helpers de detección de inscripciones — sin cambios
...
<template>
  <div class="w-full px-4 md:px-6 lg:px-8 pb-24 md:pb-8">
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <!-- Filtros — v-model bindings sin cambios, estilo refinado -->
      <div class="p-5 rounded-2xl border border-slate-200 bg-white shadow-sm grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="relative">
          <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
            <span class="inline-flex items-center gap-1.5">
              <svg class="w-3 h-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z" /></svg>
              Disciplina
            </span>
          </label>
          <select
            ...
            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400 transition-all appearance-none cursor-pointer"
          >
        ...
        <div class="relative">
          <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
            <span class="inline-flex items-center gap-1.5">
              <svg class="w-3 h-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              Instructor
            </span>
          </label>
          <select
        ...
        <div class="relative">
          <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
            <span class="inline-flex items-center gap-1.5">
              <svg class="w-3 h-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75" /></svg>
              Día de la Semana
            </span>
          </label>
          <select
        ...
        <div class="flex gap-2 items-end">
          <div class="relative flex-1">
            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
              <span class="inline-flex items-center gap-1.5">
                <svg class="w-3 h-3 text-teal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                Hora
              </span>
            </label>
            <select
          ...
          <!-- v-if store.hayFiltrosActivos / @click store.resetFiltros() sin cambios -->
          <button
      ...
      <div v-if="store.loading" class="flex flex-col items-center py-20">
        <div class="w-12 h-12 rounded-full border-4 border-slate-200 border-t-teal-500 animate-spin mb-4" />
        <p class="text-slate-400 font-semibold text-sm">Cargando actividades abiertas...</p>
      </div>
      ...
      <!-- Empty state — store.sesionesTipoAbierta.length / store.hayFiltrosActivos sin cambios -->
      <div
      ...
      >
        <div class="w-14 h-14 bg-teal-50 rounded-full flex items-center justify-center text-teal-500 mb-4 border border-teal-100">
          <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
          </svg>
        </div>
        <p class="text-slate-800 font-extrabold text-lg">Sin actividades abiertas</p>
        <p class="text-slate-400 font-medium text-sm text-center max-w-xs mt-1.5 leading-relaxed">
          {{ store.hayFiltrosActivos ? 'No hay resultados con los filtros actuales.' : 'No hay actividades abiertas programadas.' }}
        </p>
        <button
          v-if="store.hayFiltrosActivos"
          @click="store.resetFiltros()"
          class="mt-5 px-5 py-2 rounded-xl text-sm font-bold text-white bg-teal-600 hover:bg-teal-500 transition-all cursor-pointer shadow-sm"
        >
          Limpiar filtros
        </button>
      </div>

      <!-- Grid de sesiones — store.sesionesTipoAbierta / :key sesion.id_sesion sin cambios -->
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <div
          v-for="sesion in store.sesionesTipoAbierta"
          :key="sesion.id_sesion"
          class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden flex flex-col border-l-4 border-l-teal-500"
        >
          ...
          <div class="px-5 py-3.5 flex-1 flex flex-col justify-between gap-2.5">
            <div class="space-y-2">
              ...
              <div class="flex items-center gap-2 text-slate-600">
                <div class="w-6 h-6 rounded-lg bg-teal-50 flex items-center justify-center text-teal-500 shrink-0">
                  <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
              ...
              <!-- v-if sesion.instructor?.nombre sin cambios -->
              ...
              <!-- v-if sesion.espacio sin cambios -->
              <div v-if="sesion.espacio" class="flex items-center gap-2 text-slate-600">
                <div class="w-6 h-6 rounded-lg bg-violet-50 flex items-center justify-center text-violet-400 shrink-0">
                  <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
              ...
              <!-- Conteo de inscritos — sin badge "SIN LÍMITE" -->
              <div class="flex items-center gap-2 text-slate-600">
                ...
                <span class="text-sm font-semibold text-slate-500">{{ sesion.cantidad_inscritos }} inscritos</span>
              </div>
            </div>
            ...
            <div class="pt-2">
              ...
              <button
                ...
                class="w-full flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl px-4 py-2.5 text-xs font-bold transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed border border-teal-700 focus:outline-none cursor-pointer"
              >
    ...
    <!-- Modal de Inscripción — selectedSesionInscripcion / lógica inscripcionTipo sin cambios -->
    <Teleport to="body">
        ...
        <div v-if="selectedSesionInscripcion" class="fixed inset-0 z-99999 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div class="relative w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl border border-slate-200 bg-white shadow-2xl overflow-hidden flex flex-col text-slate-800 max-h-[92vh]">
            ...
            <div class="flex flex-col gap-4 p-6 overflow-y-auto">
              ...
              <!-- Listas de selección — toda la lógica :class / @click / v-if sin cambios -->
              <div class="min-h-20 flex flex-col justify-center gap-3">
                ...
                <!-- Familiar — familyStore.miembrosFamiliares / esFamiliarInscrito() / isSelected() sin cambios -->
                ...
                <!-- Invitado — guestStore.invitados / esInvitadoInscrito() sin cambios -->
            ...
            <!-- Botones de acción — handleConfirmarInscripcion / isConfirmarDisabled sin cambios -->

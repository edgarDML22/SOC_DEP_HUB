...
const fetchSocioDetails = async () => {
    ...
    errorMsg.value = '';
    try {
        const response = await api.get(`/socios/${socioId}`);
        if (response.data && response.data.success) {
            socio.value = response.data.data;
            // Poblar formulario
            editForm.value = {
        ...
        console.error("Error cargando detalles del socio:", error);
...
const saveSocioUpdates = async () => {
    isSaving.value = true;
    try {
        const res = await api.put(`/socios/update/${socioId}`, editForm.value);
        if (res.data.success) {
            showEditModal.value = false;
            // Refrescar con los nuevos datos
            socio.value = res.data.data;
        }
    } catch (error) {
        ...
        <section v-if="socio && !isLoading" class="detail-content">
            ...
            <article class="featured-card">
                ...
                <div class="featured-body">
                    ...
                    <div class="mt-6 border-t pt-4">
                        ...
                        <div class="penalties-grid">
                            <div class="penalty-card">
                                ...
                                <div class="penalty-info">
                                    <span class="penalty-count" :class="{'text-red-600': socio.contador_no_shows > 0}">{{ socio.contador_no_shows }}</span>
                            ...
                            <div class="penalty-card">
                                ...
                                <div class="penalty-info">
                                    <span class="penalty-count" :class="{'text-yellow-600': socio.retrasos_ludoteca > 0}">{{ socio.retrasos_ludoteca }}</span>
                        ...
                        <div v-if="socio.contador_no_shows >= 3 && socio.estatus_cuenta !== 'PENALIZADO' && socio.estatus_cuenta !== 'SUSPENDIDO'" class="alert-penalty">
                            ...
                            <div class="alert-penalty-body">
                                ...
                                <p class="alert-penalty-desc">Este socio tiene <strong>{{ socio.contador_no_shows }} No Shows</strong>. Puedes aplicarle una penalización de 7 días desde "Gestionar Socio".</p>
                            </div>
                            <button @click="editForm.estatus_cuenta = 'PENALIZADO'; showEditModal = true" class="alert-penalty-btn">
                                Penalizar
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="actions-group mt-8">
                        <button @click="showEditModal = true" class="btn-primary w-full">Gestionar Socio / Penalizaciones</button>
                    </div>
                </div>
            </article>
            ...
            <article class="featured-card mt-6" v-if="socio.modalidad_plan === 'FAMILIAR'">
                <div class="featured-body">
                    ...
                    <div v-if="!socio.miembros_familiares || socio.miembros_familiares.length === 0" class="empty-state text-center py-6 text-gray-500 italic">
                    ...
                    <div class="form-section mt-4 pt-4 border-t">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select v-model="editForm.estatus_cuenta" class="status-select font-bold" :class="{
                                ...
                                'text-orange-600': editForm.estatus_cuenta === 'PENALIZADO',
                                ...
                                <option value="PENALIZADO">🔒 PENALIZADO (7 días)</option>
...
.card-accent-bar { height: 6px; }
.bar-success { background: linear-gradient(90deg, #10b981 0%, #34d399 100%); }
.bar-warning { background: linear-gradient(90deg, #f59e0b 0%, #fbbf24 100%); }
.bar-danger { background: linear-gradient(90deg, #ef4444 0%, #f87171 100%); }
...
.penalty-count { font-size: 1.25rem; font-weight: 800; line-height: 1; }
.penalty-name { font-size: 0.75rem; color: var(--p-surface-500, #64748b); font-weight: 600; }
...
.family-info h4 { margin: 0; font-size: 0.95rem; color: #111827; }
.family-info p { margin: 0; font-size: 0.75rem; color: #64748b; }
...
.btn-secondary {
    background-color: white; color: var(--p-surface-700, #334155);
    border: 1px solid var(--p-surface-300, #cbd5e1); padding: 0.5rem 1rem; border-radius: 8px;
    font-weight: 600; cursor: pointer; transition: all 0.2s;
}
.btn-secondary:hover { background-color: var(--p-surface-100, #f1f5f9); }
...
.border-t { border-top: 1px solid var(--p-surface-200, #e2e8f0); }
...
.modal-content {
    ...
    border-radius: 16px; display: flex; flex-direction: column; max-height: 90vh;
}

.modal-header {
    padding: 1.25rem 1.5rem; border-bottom: 1px solid var(--p-surface-200, #e2e8f0);
...
.section-title { font-size: 0.95rem; font-weight: 700; margin-bottom: 0.75rem; color: #111827; }

.form-group { display: flex; flex-direction: column; gap: 0.4rem; margin-bottom: 0.75rem;}
.form-group label { font-size: 0.85rem; font-weight: 600; color: var(--p-surface-700, #334155); }
.form-group input, .form-group select {
    padding: 0.6rem; border-radius: 8px; border: 1px solid var(--p-surface-300, #cbd5e1); outline: none;
}
.form-group input:focus, .form-group select:focus { border-color: var(--p-primary-500, #3b82f6); }
...
.counter-input button {
    background: var(--p-surface-100, #f1f5f9); border: none; padding: 0.5rem 1rem;
    font-weight: bold; color: var(--p-surface-700, #334155); cursor: pointer;
}
.counter-input button:disabled { opacity: 0.5; cursor: not-allowed; }
.counter-input input {
...
.modal-footer {
    padding: 1.25rem 1.5rem; border-top: 1px solid var(--p-surface-200, #e2e8f0);
    display: flex; justify-content: flex-end; gap: 1rem;
}

/* Loading y Error */
.loading-state { text-align: center; padding: 3rem 1rem; display: flex; flex-direction: column; align-items: center; gap: 1rem; }
.spinner { width: 40px; height: 40px; border: 4px solid var(--p-surface-200, #e2e8f0); border-top-color: var(--p-primary-600, #2563eb); border-radius: 50%; animation: spin 1s linear infinite; }
@keyframes spin { 100% { transform: rotate(360deg); } }
.error-state { text-align: center; padding: 3rem 1rem; color: #ef4444; font-weight: bold; }
...
.alert-penalty-icon { font-size: 1.5rem; flex-shrink: 0; }

.alert-penalty-body { flex: 1; }
...
.alert-penalty-btn:hover { background-color: #dc2626; }
</style>

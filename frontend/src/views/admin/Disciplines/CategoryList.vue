...
<template>
    <div class="admin-container p-6 bg-gray-50 min-h-screen">
        <header class="flex justify-between items-center mb-8">
            ...
            <button @click="openCreate" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-indigo-200 flex items-center gap-2">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="cat in categories" :key="cat.id" class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col">
                <div class="flex justify-between items-start mb-4">
                    <div class="p-3 bg-indigo-50 text-indigo-600 rounded-xl">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 11h.01M7 15h.01M13 7h.01M13 11h.01M13 15h.01M17 7h.01M17 11h.01M17 15h.01" /></svg>
                    </div>
                    <div class="flex gap-2">
                        <button @click="openEdit(cat)" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Editar">
                        ...
                        <button @click="remove(cat.id)" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Eliminar">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">{{ cat.nombre_categoria }}</h3>
                <p class="text-gray-500 text-sm flex-grow">{{ cat.descripcion_categoria || 'Sin descripción.' }}</p>
            </div>
        </div>
            ...
            <div class="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
                <div class="p-8">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold text-gray-900">{{ editingCategory ? 'Editar Categoría' : 'Nueva Categoría' }}</h2>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input v-model="form.nombre_categoria" type="text" placeholder="Ej. ACUATICO" class="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">Descripción</label>
                            <textarea v-model="form.descripcion_categoria" rows="3" placeholder="Breve descripción..." class="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 resize-none"></textarea>
                        </div>
                    </div>
                    <div class="mt-8 flex gap-3">
                        <button @click="showModal = false" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-xl font-bold transition-all">Cancelar</button>
                        <button @click="save" :disabled="isSaving" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold transition-all shadow-lg shadow-indigo-100 disabled:opacity-50">

...
const confirmCancel = async () => {
  ...
  try {
    await api.post('torneos/update-status', {
      nombre_torneo:    selectedTorneo.value.nombre_torneo,
      fecha_inicio:     selectedTorneo.value.fecha_inicio,
      nombre_categoria: selectedTorneo.value.categoria,
      nombre_disciplina: selectedTorneo.value.disciplina,
      estado:           'CANCELADO',
    })
    ...
    toastInfo('Error', e.response?.data?.message ?? 'No se pudo cancelar el torneo.', 'error')
  } finally {
...
<template>
  <main class="min-h-screen bg-slate-50 p-6 lg:p-8 pb-16 font-sans">
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <AdminPageHeader
        title="Torneos"
        subtitle="Administra los torneos y competencias del club."
      >
        ...
        <button
          ...
          class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 text-white
      ...
      <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-4">
        <div class="relative">
          ...
          <input
            v-model="search"
            placeholder="Buscar por nombre, disciplina o categoría…"
      ...
      <!-- VACÍO -->
      <div v-else-if="filteredTorneos.length === 0"
        class="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-16
      ...
      >
        <div
          v-for="torneo in filteredTorneos"
          :key="torneo.id_torneo"
          class="bg-white rounded-3xl border border-t-4 border-slate-200 shadow-sm
          ...
          :class="estadoAccent(torneo.estado)"
        >
          <!-- Header card -->

...
async function fetchAsistencia(id_sesion) {
  ...
  try {
    ...
    // Sincronizar contador en la fila de la tabla sin re-fetch completo
    const idx = sesiones.value.findIndex(s => s.id_sesion === id_sesion)
    if (idx !== -1) sesiones.value[idx].cantidad_inscritos = asistencia.value.length
    if (sesionSeleccionada.value?.id_sesion === id_sesion) {
      sesionSeleccionada.value.cantidad_inscritos = asistencia.value.length
    }
  } catch {
    asistencia.value = []
  } finally {
                ...
                <div class="flex items-start gap-3 bg-slate-50 border border-slate-100 rounded-2xl p-3.5">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="text-xs font-semibold text-slate-500 mt-0.5">
                      {{ sesionSeleccionada?.requiere_inscripcion ? `de ${sesionSeleccionada?.cupo_maximo ?? '—'} cupo` : 'alumnos' }}
                    </p>
                  </div>
                </div>

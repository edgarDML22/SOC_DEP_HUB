...
<template>
  <div class="w-full font-sans bg-surface-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-4 md:pt-6 lg:pt-8 bg-surface-50">
      ...
      <div class="flex p-1.5 bg-surface-100 rounded-2xl w-full max-w-2xl mx-auto overflow-x-auto scrollbar-thin shadow-inner border border-surface-200 mb-8">
        <button
          v-for="tab in tabs"
          :key="tab.key"
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div v-else>
          ...
          <div v-if="activeView === 'inscripcion'">
            ...
            <div v-if="torneoStore.disponibles.length === 0" class="flex flex-col items-center py-20 bg-white rounded-3xl border border-surface-100 shadow-sm text-center px-4">
              <div class="w-16 h-16 bg-primary-50 rounded-full flex items-center justify-center text-primary-500 mb-4 shadow-sm">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div
                v-for="torneo in torneoStore.disponibles"
              ...
              >
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex justify-between items-start pt-1">
                    ...
                    <span class="bg-primary-50 text-primary-700 text-[10px] font-extrabold px-2.5 py-1 rounded-full uppercase tracking-wider border border-primary-100">
                      {{ torneo.tipo_acceso }}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <p class="text-xs font-semibold text-surface-400 mt-1">
                      Categoría: <span class="text-surface-600 font-extrabold">{{ torneo.categoria?.nombre_categoria }}</span>
                    </p>
                  </div>

                  <!-- Details Badges (Age/Gender) -->
                  <div class="flex flex-wrap gap-2 text-[11px] font-bold">
                    <span class="bg-surface-50 border border-surface-150 text-surface-600 px-2.5 py-1 rounded-lg flex items-center gap-1">
                  ...
                  <div class="bg-surface-50/50 rounded-2xl p-3 border border-surface-100 flex items-center gap-3">
                    ...
                    <div class="text-[11px] font-semibold text-surface-500 space-y-0.5">
                      <p>Inicio: <span class="font-extrabold text-surface-700">{{ formatDate(torneo.fecha_inicio) }}</span></p>
                      <p>Cierre: <span class="font-extrabold text-surface-700">{{ formatDate(torneo.fecha_fin) }}</span></p>
                    </div>
                  </div>
                ...
                <div class="mt-6">
                  ...
                  <button
                    v-else-if="torneo.inscritos_actual >= torneo.cupo_maximo"
                    disabled
          ...
          <div v-else-if="activeView === 'historial'">
            ...
            <div v-if="torneoStore.historial.length === 0" class="flex flex-col items-center py-20 bg-white rounded-3xl border border-surface-100 shadow-sm text-center px-4">
              <div class="w-16 h-16 bg-surface-50 rounded-full flex items-center justify-center text-surface-400 mb-4 border border-surface-100">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- History Card -->
              <div
                v-for="item in torneoStore.historial"
                :key="item.id_torneo"
              ...
              >
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex justify-between items-start pt-1 pl-1">
                    ...
                    >
                      {{ item.estatus_torneo === 'EN_CURSO' ? 'En Juego' : (item.estatus_torneo === 'EN_INSCRIPCION' ? 'Abierto' : 'Finalizado') }}
                    </span>
                  </div>
                  ...
                  <div class="pl-1 text-xs font-semibold text-surface-400 flex items-center gap-1.5">
                    ...
                    Fecha inicio: <span class="font-extrabold text-surface-600">{{ formatDate(item.fecha_inicio) }}</span>
                  </div>
                  ...
                  <div class="pt-2 pl-1 border-t border-surface-100 flex items-center gap-2">
                    ...
                    <span
                      v-if="item.fase_maxima_alcanzada"

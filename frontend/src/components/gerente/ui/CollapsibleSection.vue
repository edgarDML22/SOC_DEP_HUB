...
<template>
  <section class="border-b border-slate-100 last:border-b-0">
    <button
      type="button"
      @click="toggle"
      class="w-full flex items-center gap-3 text-left transition-colors hover:bg-slate-50/60 focus:outline-none focus:bg-slate-50"
      :class="dense ? 'px-5 py-3' : 'px-6 py-4'"
    >
      <svg
    ...
    >
      <div :class="dense ? 'px-5 pb-4' : 'px-6 pb-5'">
        <slot />
      </div>
    </div>
  </section>

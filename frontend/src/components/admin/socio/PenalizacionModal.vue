        ...
        <Transition enter-active-class="transition-all duration-300 ease-out"
          ...
          <div v-if="modelValue"
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs font-extrabold text-surface-500 mt-0.5 uppercase tracking-widest truncate max-w-xs">
                    ...
                    <span class="font-mono text-surface-400 normal-case tracking-normal">#{{ socio?.numero_accion
                      }}</span>
                  </p>
                </div>
              </div>
              <button @click="close" class="w-9 h-9 rounded-xl bg-surface-100 hover:bg-surface-200
            ...
            <div class="overflow-y-auto bg-surface-50/40">
              <div class="p-6 grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-6">
                ...
                <div class="flex flex-col gap-5">
                  <div>
                    <p class="text-[10px] font-black uppercase tracking-widest text-surface-500 mb-3">Historial del
                      Socio</p>
                    ...
                    <div class="bg-white rounded-2xl border border-surface-200 shadow-sm overflow-hidden">
                      ...
                      <div class="p-5 flex items-center justify-between">
                        <div>
                          <p class="text-[10px] font-black uppercase tracking-widest text-surface-500 mb-1">Retrasos
                            Ludoteca</p>
                          <p class="text-3xl font-black leading-none transition-colors"
                  ...
                  <div class="bg-white rounded-2xl border border-surface-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-3 border-b border-surface-100">
                      <p class="text-[10px] font-black uppercase tracking-widest text-surface-500">Duración de la
                        Sanción</p>
                    </div>
                    <div class="p-5 flex flex-col gap-4">
                      ...
                      <div>
                        <div class="flex items-center justify-between mb-2">
                          ...
                          <span v-if="!reservaActiva"
                            class="text-[10px] font-bold text-surface-300 uppercase tracking-wide">
                            No aplica
                          </span>
                        </div>
                        <div class="flex gap-2">
                          <button v-for="d in DIAS_OPTIONS" :key="'r' + d" :disabled="!reservaActiva || isSaving"
                            @click="diasReserva = d" class="flex-1 py-2 rounded-xl text-xs font-black border-2 transition-all duration-150
                      ...
                      <div>
                        <div class="flex items-center justify-between mb-2">
                          ...
                          <span v-if="!ludotecaActiva"
                            class="text-[10px] font-bold text-surface-300 uppercase tracking-wide">
                            No aplica
                          </span>
                        </div>
                        <div class="flex gap-2">
                          <button v-for="d in DIAS_OPTIONS" :key="'l' + d" :disabled="!ludotecaActiva || isSaving"
                            @click="diasLudoteca = d" class="flex-1 py-2 rounded-xl text-xs font-black border-2 transition-all duration-150
                                   disabled:opacity-30 disabled:cursor-not-allowed"
                ...
                <div class="flex flex-col gap-4">
                  <p class="text-[10px] font-black uppercase tracking-widest text-surface-500 px-1">Estatus de
                    Penalización</p>

                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <button v-for="card in PENALTY_CARDS" :key="card.status" @click="selectedStatus = card.status"
                      :disabled="isSaving" class="relative flex flex-col gap-2.5 p-4 rounded-2xl border-2 text-left
                             transition-all duration-200 disabled:opacity-40 cursor-pointer
                             hover:-translate-y-0.5 hover:shadow-md focus:outline-none" :class="selectedStatus === card.status
                      ...
                      <div class="flex items-center justify-between">
                        <div class="w-9 h-9 rounded-xl flex items-center justify-center transition-colors" :class="selectedStatus === card.status
                  ...
                  <div class="mt-1 rounded-2xl bg-white border border-surface-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-3 flex items-center gap-3 border-b border-surface-100">
                      ...
                      <div class="flex-1 min-w-0">
                        <p class="text-[10px] font-black uppercase tracking-widest text-surface-400">Penalización a
                          aplicar</p>
                        <p class="text-sm font-black text-surface-900 truncate">{{ activeCard?.label ?? selectedStatus
                          }}</p>
                      </div>
                    ...
                    <div v-if="selectedStatus !== 'SIN_PENALIZACION'" class="px-5 py-3 flex gap-6">
                      <div v-if="reservaActiva" class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-black text-red-600">{{ diasReserva }} día{{ diasReserva > 1 ? 's' : ''
                          }}</span>
                      </div>
                      <div v-if="ludotecaActiva" class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-black text-amber-600">{{ diasLudoteca }} día{{ diasLudoteca > 1 ? 's'
                          : '' }}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div class="flex items-center justify-end gap-3 px-7 py-4 border-t border-surface-100 bg-white">
              <button @click="close" class="px-5 py-2.5 rounded-xl border border-surface-200 bg-white
                       text-sm font-bold text-surface-700 hover:bg-surface-50 transition-colors">
                Cancelar
              </button>
              <button @click="save" :disabled="isSaving || !hasChanges" class="px-6 py-2.5 rounded-xl bg-primary-600 text-white text-sm font-black
                       hover:bg-primary-700 active:bg-primary-800 transition-colors shadow-sm
                       disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
                <svg v-if="isSaving" class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  stroke-width="2.5">
